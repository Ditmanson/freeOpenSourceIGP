...
- Function URL CORS config (updated in ticket 06, see that ticket's notes for why): `AllowOrigins: ["https://griz.sh"]`, `AllowMethods: ["POST"]`, `AllowHeaders: ["content-type", "hx-request", "hx-trigger", "hx-trigger-name", "hx-target", "hx-current-url"]` - the original `content-type`-only list set here in ticket 01 was correct for a bare curl/fetch client, but not for the actual htmx-based widget built in ticket 05, which always attaches its own `hx-*` headers
